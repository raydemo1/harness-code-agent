...
description: Guidance for developing CoreWars warriors that achieve target win rates against specific opponents. This skill should be used when tasks involve writing, optimizing, or debugging Redcode assembly warr
---
...
This skill provides a practical, repeatable methodology for developing CoreWars Redcode warriors that meet specified win-rate targets against multiple opponents. It emphasizes systematic opponent analysis, minimal starting designs, automated testing, and hypothesis-driven parameter tuning to reach reliable thresholds. The goal is measurable, explainable improvements rather than ad-hoc trial-and-error.

How this skill works

The skill inspects opponent source code to extract types, step sizes, timing, and exploitable patterns, then selects an archetype (scanner, bomber, replicator, imp, vampire, stone, etc.) that counters those patterns. It prescribes an iterative workflow: implement a minimal warrior, run automated battle tests, profile failures with pmars, and perform targeted parameter searches (grid/binary) until required win rates are met. All changes are tracked and justified.

When to use it

When you must reach specific win-rate thresholds against a list of opponents

When writing or optimizin
