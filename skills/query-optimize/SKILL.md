---
name: query-optimize
description: Guidance for SQL query optimization tasks. This skill should be used when optimizing slow SQL queries, improving database performance, or rewriting queries to be more efficient. Covers query plan anal
---
...
This skill provides practical guidance for optimizing slow SQL queries and improving database performance. It focuses on establishing baselines, analyzing query plans, applying multiple optimization techniques, and verifying both correctness and performance. Use it to produce repeatable, benchmarked improvements and clear documentation of why changes help.

How this skill works

The skill guides you through a structured workflow: measure the original query, inspect EXPLAIN/EXPLAIN ANALYZE plans, identify bottlenecks (full scans, correlated subqueries, missing indexes), and propose several alternative rewrites. It covers techniques such as CTEs, window functions, join reordering, and database-specific adjustments for SQLite, PostgreSQL, and MySQL. Final verification uses full-result comparisons, multi-run timing, and cold vs warm cache checks.

When to use it

A query is consistently slow or causing timeouts

You need to reduce resource consumption or latency on a critical report

Refactoring queries after a schema or data-volume change

Before adding hardware: check if SQL-level improvements exist

Validating that a new query produces identical results with better performance

Best practices

Establish clear performance goals and measure baseline with multiple runs

Always run EXPLAIN (or EXPLAIN ANALYZE) before and after changes

Test at least 2–3 alternative approaches rather than accepting the first improvement

Verify full correctness via row counts, checksums, or full-result comparisons

Test both cold and warm cache and use median times from multiple runs

Account for database-specific behaviors (CTE materialization, parallelism, optimizer hints)

Example use cases

Convert a correlated subquery to a JOIN or CTE and measure latency reduction

Add or adjust indexes for slow WHERE, JOIN, or ORDER BY columns and validate plan changes

Rewrite a GROUP BY aggregation to filter earlier and reduce intermediate rows

Use window functions to replace inefficient row-by-row logic for ranking or running totals

Benchmark and choose between a materialized CTE, derived table, or inline expression on a specific DBMS

FAQ

How many runs should I
